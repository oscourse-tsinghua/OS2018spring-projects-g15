...
impl<T: ?Sized> ArefBorrow<T>
{
	...
	pub fn __inner(&self) -> &ArefInner<T> {
		// SAFE: Nobody gets a &mut to the inner, and pointer should be valid
		unsafe { self.__ptr.as_ref() }
	}
	pub fn get_data(&mut self) -> &mut T {
		// SAFE: Nobody gets a &mut to the inner, and pointer should be valid
		//unsafe { (self.__ptr.as_ref() as *mut ArefInner<T>).data }
		unsafe { &mut (*(self.__ptr.as_ptr() as *mut ArefInner<T>)).data }
	}
}
