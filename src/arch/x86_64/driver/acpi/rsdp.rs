...
impl RSDP {
    ...
    pub fn get_rsdp(active_table: &mut ActivePageTable) -> Option<RSDP> {
        ...
        {
            let start_frame = Frame::containing_address(start_addr as usize);
            let end_frame = Frame::containing_address(end_addr as usize);
            for frame in Frame::range_inclusive(start_frame, end_frame) {
                let page = Page::containing_address(frame.start_address().0 as usize);
