...
fn get_sdt(sdt_address: usize, active_table: &mut ActivePageTable) -> &'static Sdt {
    {
        let page = Page::containing_address(sdt_address as usize);
        if active_table.translate_page(page).is_none() {
            let frame = Frame::containing_address(page.start_address() as usize);
    ...
    {
        let start_page = Page::containing_address(sdt_address + 4096 as usize);
