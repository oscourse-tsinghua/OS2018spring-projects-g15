...
struct AtaRegs
{
	ata_base: u16,
	sts_base: u16,
	prdts: ::memory::virt::ArrayHandle<PRDTEnt>,
}
...
impl AtaRegs
{
	fn new(ata_base: u16, sts_port: u16) -> AtaRegs
	{
		AtaRegs {
			...
			prdts: ::memory::virt::alloc_dma(32, 1, module_path!()).unwrap().into_array(),
		}
	}
	...
	{
		assert!(ofs < 8);
		port::outb( self.ata_base + ofs, val);
	}
	unsafe fn out_16(&mut self, ofs: u16, val: u16)
	{
		assert!(ofs < 8);
		port::outw( self.ata_base + ofs, val);
	}
	
	unsafe fn in_8(&mut self, ofs: u16) -> u8
	{
		assert!(ofs < 8);
		port::inb( self.ata_base + ofs )
	}
	unsafe fn in_16(&mut self, ofs: u16) -> u16
	{
		assert!(ofs < 8);
		port::inw( self.ata_base + ofs )
	}
		...
		let mut count = 0;
		{
			...
			for region in dma_buffer.phys_ranges()
			{
				...
				while bytes > 0
				{
					...
					paddr += ent_bytes as ::memory::PAddr;
					bytes -= ent_bytes;
				}
			}
		}
	...
	fn start_dma(&mut self, disk: u8, blockidx: u64, dma_buffer: &DMABuffer, is_write: bool, bm: &DmaRegBorrow)
	{
		...
		unsafe
		{
			...
			// - Set PRDT
			bm.out_32(4, ::memory::virt::get_phys(&self.prdts[0]) as u32);
...
impl AtaController
{
	...
	pub fn ata_identify<'a>(&'a self, disk: u8, data: &'a mut super::AtaIdentifyData, class: &'a mut super::AtaClass) -> async::poll::Waiter<'a>
	{
