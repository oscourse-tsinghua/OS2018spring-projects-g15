...
impl HeapDef
{
	...
	pub unsafe fn allocate(&mut self, size: usize, align: usize) -> Result<*mut (), Error>
	{
		...
		let blocksize = ::mylib::num::round_up(size + headers_size, 32);
	...
	pub unsafe fn deallocate(&mut self, ptr: *mut (), size: usize, _align: usize)
	{
		...
		assert!(headptr as usize >= HEAP_START);
		
		{
			...
			// 1. Left:
			if headptr as usize != HEAP_START
			{
	...
	unsafe fn expand(&mut self, min_size: usize) -> Result<*mut HeapHead, Error>
	{
		...
		let n_pages = ::mylib::num::round_up(alloc_size, super::super::PAGE_SIZE) / super::super::PAGE_SIZE;
		////log_debug!("HeapDef.expand(min_size={}), alloc_size={}, n_pages={}", min_size, alloc_size, n_pages);
		////log_trace!("last_foot = {:p}", self.last_foot);
		assert!(n_pages > 0);

		if last_foot.next_head() as usize == HEAP_START + HEAP_SIZE-1 {
			return Err( Error::OutOfReservation );
		}
		if last_foot.next_head() as usize + n_pages * super::super::PAGE_SIZE > HEAP_START + HEAP_SIZE-1 {
			return Err( Error::OutOfReservation );
		}
		...
		let block = if use_prev
			{
				...
				let newsize = block.size() + n_pages * super::super::PAGE_SIZE;
			...
			else
			{
				...
				(*block).initialise( n_pages * super::super::PAGE_SIZE, HeapState::Free(0 as *mut _) );
