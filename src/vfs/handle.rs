...
impl Dir
{
	...
	/// Create a new file
	pub fn mkfile(&self, name: &str, mode: FileOpenMode) -> super::Result<File> {
		let node = try!(self.node.create(name.as_ref(), NodeType::File));
		assert!(node.is_file());
		Ok( File { node: node, mode: mode } )
	}
